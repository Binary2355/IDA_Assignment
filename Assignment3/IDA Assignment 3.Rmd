...
$$
\hat{y}^{(m)}_{mis}=X_{mis}\theta+z,\quad z\sim N(0,\hat{\sigma}^2),\,m=1,2,\dots,M
$$
Then:

For case (i), first compute $\hat{\theta}^{(m)}$ for $m=1,2,\dots,M$, then using Rubin's Rules, pool the estimate $\hat{\theta}^{MI}=\frac{1}{M}\sum\limits_{m=1}^M\hat{\theta}^{(m)}$. Therefore, the predicted values of $$
